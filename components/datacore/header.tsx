...
export default function Header({ onMenuToggle, sidebarOpen }: { onMenuToggle?: () => void; sidebarOpen?: boolean }) {
  const {
    ...
    refreshStats, // 👈 AGREGAMOS LA NUEVA FUNCIÓN
  } = useIntel();
  ...
  const handleImport = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 🔥 ACTUALIZACIÓN DIRECTA SOLO DE ESTADÍSTICAS
      try {
        const newStats = await refreshStats();
        if (newStats) {
          toast.info(`Contador actualizado: ${newStats.totalCount.toLocaleString()} registros`, { duration: 1500 });
        } else {
          // Fallback: recargar datos completos
          await refreshData();
        }
      } catch (error) {
        console.error('Error actualizando estadísticas:', error);
        // Fallback final: recargar la página
        toast.warning('Recargando página para actualizar datos...', { duration: 1500 });
        setTimeout(() => {
